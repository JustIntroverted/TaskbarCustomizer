...
namespace TaskbarCustomizer {
    ...
    public partial class MainWindow : Window {
        ...
        private TaskbarElement[] _taskbarButtons = new TaskbarElement[3];
        ...
        public void CreateSettings()
        {
            ...
            {
                SettingName = "red",
                SettingValue = SliderDummyTaskRed.Value.ToString()
            };
            _settings.AddSetting(setting);
            ...
            {
                SettingName = "green",
                SettingValue = SliderDummyTaskGreen.Value.ToString()
            };
            _settings.AddSetting(setting);
            ...
            {
                SettingName = "blue",
                SettingValue = SliderDummyTaskBlue.Value.ToString()
            };
            _settings.AddSetting(setting);
            ...
            {
                SettingName = "opacity",
                SettingValue = SliderDummyTaskOpacity.Value.ToString()
            };
            _settings.AddSetting(setting);
        ...
        private void ApplyStyle()
        {
            ...
            //_dummyTaskbar.Background = new SolidColorBrush(Color.FromArgb((byte)SliderDummyTaskOpacity.Value, 0, 0, 0));
            UpdateDummySliders();

            // get the offsets of buttons that may or may not be visible
            int offset = 0; // (_startButton.IsElementVisible() ? _startButton.GetWidth() : 0);

            for (int i = 0; i < 3; i++)
            {
                offset += _taskbarButtons[i].IsElementVisible() ? _taskbarButtons[i].GetWidth() : 0;
            }

            // resize the app container and then move it into position
            _mainAppContainer.ResizeElement((int)_dummyTaskbar.Width - _trayIconContainer.GetWidth() - offset - 32);
            _mainAppContainer.MoveElement((int)_dummyTaskbar.Left + offset + 32);
            ...
            // move taskbar buttons into position
            for (int i = 0; i < 3; i++)
            {
                if (_taskbarButtons[i].IsElementVisible())
                    _taskbarButtons[i].MoveElement((int)_dummyTaskbar.Left + offset - (_taskbarButtons[i].GetWidth() * i));
            }
        ...
        private void Window_Closing(object sender, CancelEventArgs e)
        {
            ...
            _settings.UpdateSetting(new Setting("red", ((int)SliderDummyTaskRed.Value).ToString()));
            _settings.UpdateSetting(new Setting("green", ((int)SliderDummyTaskGreen.Value).ToString()));
            _settings.UpdateSetting(new Setting("blue", ((int)SliderDummyTaskBlue.Value).ToString()));
            _settings.UpdateSetting(new Setting("opacity", ((int)SliderDummyTaskOpacity.Value).ToString()));
        ...
        private void SliderDummyTaskOpacity_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            UpdateDummySliders();
        }

        private void SliderDummyTaskRed_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            UpdateDummySliders();
        }

        private void SliderDummyTaskGreen_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            UpdateDummySliders();
        }

        private void SliderDummyTaskBlue_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            UpdateDummySliders();
        }

        private void UpdateDummySliders()
        {
            if (_running)
                _dummyTaskbar.Background = new SolidColorBrush(Color.FromArgb(
                    (byte)SliderDummyTaskOpacity.Value,
                    (byte)SliderDummyTaskRed.Value,
                    (byte)SliderDummyTaskGreen.Value,
                    (byte)SliderDummyTaskBlue.Value));
        }
